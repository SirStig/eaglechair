...
class Finish(Base):
    ...
    color_id = Column(
        Integer, ForeignKey("colors.id", ondelete="SET NULL"), nullable=True
    )
...
class Chair(Base):
    ...
    flame_certifications = Column(
        JSON, nullable=True
    )  # e.g., ["CAL 117", "UFAC Class 1"]
    ...
    # Usage & Application
    recommended_use = Column(
        String(255), nullable=True
    )  # e.g., "Restaurant", "Healthcare"
    is_outdoor_suitable = Column(
...
class ProductRelation(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    product_id = Column(
        Integer, ForeignKey("chairs.id", ondelete="CASCADE"), nullable=False
    )
    related_product_id = Column(
        Integer, ForeignKey("chairs.id", ondelete="CASCADE"), nullable=False
    )
    relation_type = Column(
        String(50), default="related", nullable=False
    )  # e.g., "related", "alternative", "accessory"
    display_order = Column(Integer, default=0, nullable=False)
...
class Color(Base):
    ...
    )  # e.g., "Walnut Brown", "Black", "Charcoal Gray"
    color_code = Column(
        String(50), unique=True, nullable=True
    )  # e.g., "WB-001", "BLK", "CG-100"
...
class ProductVariation(Base):
    ...
    upholstery_id = Column(
        Integer, ForeignKey("upholsteries.id", ondelete="SET NULL"), nullable=True
    )
...
class ProductImage(Base):
    ...
    variation_id = Column(
        Integer, ForeignKey("product_variations.id", ondelete="SET NULL"), nullable=True
    )
    ...
    )  # primary|hover|gallery|side|front|back|detail
...
class CustomOption(Base):
    ...
    )  # "back_handle", "special_glide", "custom_dimension", etc.
    ...
    # Pricing
    price_adjustment = Column(Integer, default=0, nullable=False)  # In cents
    requires_quote = Column(
        Boolean, default=False, nullable=False
    )  # Some options need manual quoting
...
class ProductTag(Base):
    ...
    tag_type = Column(
        String(50), nullable=True
    )  # "feature", "material", "style", "use_case"
    description = Column(Text, nullable=True)
...
class ProductTagAssociation(Base):
    ...
    product_id = Column(
        Integer, ForeignKey("chairs.id", ondelete="CASCADE"), primary_key=True
    )
    tag_id = Column(
